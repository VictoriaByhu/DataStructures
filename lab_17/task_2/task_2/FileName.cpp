...
void binarySearch(Student arr[], int SIZE, double key)
{
	...
	while (left <= right)
	{
		...
		if (arr[mid].averageMark == key)
		{
			cout << "Student " << arr[mid].Name << " has average mark " << key << endl;
			break;
		}

		else if (arr[mid].averageMark < key)
			left = mid + 1;
...
int main()
{
	...
	bubbleSort(student, SIZE);
	double key;
	cout << "Enter an average number: ";
	cin >> key;

	binarySearch(student, SIZE, key);
